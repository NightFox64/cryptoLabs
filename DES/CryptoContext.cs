...
namespace CryptoLibrary
{
    public class CryptoContext
    {
        ...
        public CryptoContext(ISymmetricCipher cipher, byte[] key, CipherMode mode, PaddingMode padding, 
        ...
        private byte[] DecryptECB(byte[] data)
        {
            ...
            Parallel.For(0, data.Length / _blockSize, i =>
            {
                ...
                Array.Copy(decrypted, 0, result, i * _blockSize, _blockSize);
            });
            return result;
        }
        ...
        {
            byte[] result = new byte[data.Length];
            byte[] previousXor = (byte[])_iv.Clone();
            ...
            for (int i = 0; i < data.Length; i += _blockSize)
            {
                ...
                XorBlocks(block, previousXor);
                byte[] encrypted = _cipher.Encrypt(block);
                Array.Copy(encrypted, 0, result, i, _blockSize);
                
                byte[] plainBlock = new byte[_blockSize];
                Array.Copy(data, i, plainBlock, 0, _blockSize);
                XorBlocks(plainBlock, encrypted);
                previousXor = plainBlock;
            }
            return result;
        }
        ...
        private byte[] DecryptPCBC(byte[] data)
        {
            ...
            for (int i = 0; i < data.Length; i += _blockSize)
            {
                ...
                byte[] decrypted = _cipher.Decrypt(block);
                XorBlocks(decrypted, previousXor);
                Array.Copy(decrypted, 0, result, i, _blockSize);
                
                XorBlocks(decrypted, block);
                previousXor = decrypted;
            }
            return result;
        }
        ...
        {
            return EncryptOFB(data); // OFB is symmetric
        }
        ...
        {
            return EncryptCTR(data); // CTR is symmetric
        }
        ...
        private byte[] EncryptRandomDelta(byte[] data)
        {
            ...
            for (int i = 0; i < data.Length; i += _blockSize)
            {
                ...
                XorBlocks(block, delta);
                byte[] encrypted = _cipher.Encrypt(block);
                Array.Copy(encrypted, 0, result, i, Math.Min(_blockSize, data.Length - i));
            }
            return result;
        }
        ...
        {
            throw new NotImplementedException("Random Delta decryption requires delta storage");
        }
